...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && pithead_csrf_validate((string) ($_POST['csrf'] ?? ''))) {
    ...
    if ($mid > 0) {
        $pdo->prepare('UPDATE contact_messages SET status = ? WHERE id = ?')->execute(['read', $mid]);
    }
}

$rows = $pdo->query('SELECT * FROM contact_messages ORDER BY id DESC LIMIT 200')->fetchAll();
...
<?php require __DIR__ . '/_nav.php'; ?>
  <main class="mx-auto max-w-6xl px-4 py-10">
    <h1 class="text-2xl font-bold uppercase tracking-tight">Contact messages</h1>
        ...
        <article class="border border-offwhite/15 p-6">
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p class="mt-2 text-lg font-bold"><?= pithead_h((string) $r['name']) ?></p>
              <p class="text-sm text-offwhite/80"><?= pithead_h((string) $r['email']) ?></p>
              <?php if (($r['subject'] ?? '') !== '') : ?>
                <p class="text-sm text-offwhite/60"><?= pithead_h((string) $r['subject']) ?></p>
              <?php endif; ?>
            </div>
            <?php if ($r['status'] === 'new') : ?>
              <form method="post">
