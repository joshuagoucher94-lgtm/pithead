...
**`/shop/` errors** — almost always database: wrong `db.host` / `db.name` / `db.user` / `db.pass`, or `schema.sql` not imported (missing `products` / `product_images` tables). On Hostinger, use the MySQL hostname from **hPanel → Databases**. If `config.local.php` still has `db.host` `127.0.0.1` and TCP is refused, [`db.php`](php/public/_inc/db.php) automatically retries with `localhost` (socket). Shop pages show **503** with a short message instead of a blank **500** when the DB or catalog query still fails. Check **`public_html/_inc/logs/php-errors.log`**. To see a clear JSON error: add `'health_check_key' => '…'` to `config.local.php`, open `https://pithead.co.uk/api/health-db.php?key=…`, then **remove** the key.
